...
public class NewsUntil {
	...
	Logger logger = LoggerFactory.getLogger(NewsUntil.class);
	 ...
	 * 
	 */
	public String urlPicUpload(String  content,Integer reviewId) {
		...
		for (Element element : elementImg) {
			...
			String sendPic = uploadUntil.SendPic(srcStr);
			pictureMapper.addPicture(sendPic, reviewId);
			content = content.replace(srcStr, sendPic);
		}
		return  content;
	}
	 ...
	 * 
	 */
	public void deleteLocalhostPic(Integer reviewId) {
		List<String> urList = pictureMapper.getpictureUrlByReviewId(reviewId);
		for (String string : urList) {
			uploadUntil.deletePic(string);
		}
		pictureMapper.deleteUrlByReviewId(reviewId);
	}
	...
	public List<EstateNews> getMessage(String HTML, EstateExpression estateExpression,Integer sourceId, Date thisTimeDate, HashSet<String> set) {
		...
		for(Element element : dateElements) {
			...
			try {
				...
				logger.info("网站ID：" + sourceId + "	在时间转换错误！\n" + e);
				continue;
			}
			...
			list.add(estateNew);
			String settitle = DigestUtils.md5DigestAsHex(estateNew.getTitle().getBytes());
			set.add(settitle);
		}
		return list;
	}
	...
	private EstateNews getSingleNew(String aString,EstateExpression estateExpression, HashSet<String> set)throws Exception {
		...
		if(contentHtml == null || contentHtml.equals(""))  {
			logger.info("网址：" + aString + "	HTML获取失败");
			throw new RuntimeException();
		}
		String title = this.getTitle(contentHtml, estateExpression.getTitle());
		if(title == null || title.equals(""))  {
			logger.info("网址：" + aString + "	title 获取失败");
			throw new RuntimeException();
		}
//		if(title.equals("绵兴路改造方案通过市规委会审议")) {
//			System.out.println("kaishi ");
//		}
		if(set.contains(DigestUtils.md5DigestAsHex(title.getBytes()))) {
			logger.info("网址：" + aString + "	title 已存在");
			throw new RuntimeException();
		}
		String content = this.getContent(contentHtml, estateExpression.getContent());
		if(content == null || content.equals("")) {
			logger.info("网址：" + aString + "	content 获取失败");
			throw new RuntimeException();
		}
...
//		对正文格式进行定制 和微信前端一样的展示效果,
			...
			logger.error("网址：" + aString + "	格式转化失败(和微信前端一样)\n" + e);
			throw new RuntimeException();
		}
...
//		进行对 img a 标签进行一定的处理   
			...
			logger.error("网址：" + aString + "	针对img a 标签进行处理时出现错误\n" + e);
			throw new RuntimeException();
		}
